...
//auto mode
	...
	private void Update(){
		pressedSpace += Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonUp(0) ? 1 : 0;
		if (myTransform.position.y+myCollider2D.bounds.size.y<= cam.getlowerBound() && !dead){
	...
	public void Reset()
    {
		myTransform.position = new Vector2(myTransform.position.x, cam.getlowerBound() + 10f);
